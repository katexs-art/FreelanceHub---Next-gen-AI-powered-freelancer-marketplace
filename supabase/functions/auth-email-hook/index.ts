...
function formatFromAddress(email: string): string {
  return email.includes('<') ? email : `${SITE_NAME} <${email}>`
}
...
const SAMPLE_PROJECT_URL = "https://katexs.com"
...
async function handleWebhook(req: Request): Promise<Response> {
  ...
  // Send through the linked Resend connection for this workspace. The previous
  // Lovable sender domain is not configured in this moved workspace.
  ...
  const fromEmail = 'noreply@katexs.com'
  ...
  if (!resendResponse.ok) {
    ...
    const errorMessage = `Resend ${resendResponse.status}: ${errorText}`
    console.error('Auth email send failed', { error: errorMessage, run_id, emailType })
    await supabase.from('email_send_log').insert({
      message_id: messageId,
      template_name: emailType,
      recipient_email: payload.data.email,
