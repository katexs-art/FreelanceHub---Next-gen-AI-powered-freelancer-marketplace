  ...
  try {
    ...
    switch (action) {
      case "create-assistant": {
        const { name, firstMessage, voice, model, systemPrompt, transferNumber } = params;
        const body: Record<string, unknown> = {
          name: name || "River AI",
          firstMessage: firstMessage || "Hi, thanks for calling. This is River, how can I help you today?",
          voice: { provider: "openai", voiceId: voice || "nova" },
          model: {
            provider: "openai",
        ...
        if (transferNumber) {
          body.forwardingPhoneNumber = transferNumber;
        }
        // Tag with metadata for multi-tenant
        body.metadata = { katexs_user_id: userId };
        ...
        await serviceClient.from("vapi_assistants").upsert({
          ...
          vapi_id: data.id,
          name: data.name,
          voice_provider: data.voice?.provider,
        ...
        // Deactivate other assistants for this user
        await serviceClient
      ...
      case "update-assistant": {
        const { assistantId, name, firstMessage, voice, systemPrompt, transferNumber, recordCalls, transcribeCalls } = params;
        if (!assistantId) throw new Error("assistantId required");

        const body: Record<string, unknown> = {};
        if (name) body.name = name;
        if (firstMessage) body.firstMessage = firstMessage;
        if (voice) body.voice = { provider: "openai", voiceId: voice };
        if (systemPrompt) {
          body.model = {
            provider: "openai",
            model: "gpt-4o-mini",
      ...
      case "search-numbers": {
        const { areaCode } = params;
        // Vapi doesn't have a search endpoint; we just return info that purchase will auto-assign
      ...
      case "test-call": {
        ...
        // Find the active assistant if none specified
      ...
      case "list-assistants": {
        ...
        // Filter to this user's assistants by metadata
