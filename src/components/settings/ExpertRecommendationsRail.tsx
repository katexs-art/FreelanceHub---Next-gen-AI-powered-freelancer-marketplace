...
export function ExpertRecommendationsRail() {
    ...
    (async () => {
      ...
      if (ids.length) {
        ...
        categories = Array.from(
          new Set(((gigs ?? []) as any[]).map((g) => g.primary_category).filter(Boolean)),
        );
      }
      let query = supabase
        .from("profiles")
        ...
        .limit(12);
      if (categories.length) query = query.in("primary_category", categories);
      const { data } = await query;
      if (!active) return;
      let list = (data ?? []) as Expert[];
      if (list.length < 3 && categories.length) {
  ...
  return (
    <aside className="w-[200px] shrink-0 hidden lg:block sticky top-24 self-start">
      <div className="text-xs font-mono uppercase tracking-[0.14em] text-[#6B7280] mb-3">
