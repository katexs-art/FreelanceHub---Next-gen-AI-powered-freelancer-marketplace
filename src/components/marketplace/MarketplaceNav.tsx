...
export function MarketplaceNav() {
  ...
  const dashHref = profile?.role === "expert" ? "/dashboard/expert" : "/dashboard/client";

  return (
    <header style={{
      borderBottom: `1px solid ${C.border}`, background: C.black,
      ...
      <nav style={{ display: "flex", gap: "1.5rem", alignItems: "center", fontFamily: FONT, fontSize: "0.82rem" }}>
        ...
        {!user && <Link to="/signup/expert" style={{ color: C.gray, textDecoration: "none" }}>Become an expert</Link>}
        {profile?.role === "admin" && (
          <Link to="/marketplace/admin" style={{ color: C.neon, textDecoration: "none", fontWeight: 600 }}>Admin</Link>
        )}
        {user ? (
          <>
            <Link to={dashHref} style={{ color: C.white, textDecoration: "none" }}>Dashboard</Link>
            <button onClick={signOut} style={{
              background: "transparent", border: `1px solid ${C.border2}`, color: C.white,
              borderRadius: 999, padding: "0.5rem 1.2rem", fontFamily: FONT, fontSize: "0.82rem",
              cursor: "pointer",
            }}>Sign out</button>
          </>
        ) : (
